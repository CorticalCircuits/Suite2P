...
ops.readTiffHeader = 0;%getOr(ops,'readTiffHeader',0);
...
%         ops.SubDirs{k}    = num2str(db.expts(k));
    ...
    if ~isfield(ops, 'RootDir')
        %%%SPD
        if strcmp(db.which2p,'sci')  %want to remove mega embedding of folders
            ops.RootDir = fullfile(ops.RootStorage, ops.date);
        else
...
%%%SPD
if strcmp(db.which2p,'sci')
    ops.ResultsSavePath = sprintf('%s\\%s\\%s\\%s\\', ...
        ops.ResultsSavePath, ops.fb, ops.exptChars)
else
    
    ops.ResultsSavePath = sprintf('%s\\%s\\%s\\%s\\', ...
        ops.ResultsSavePath, ops.mouse_name, ops.date, ['Site',db.site, '_',db.depth, '_',db.stimType], ops.exptChars)
end
